...
```matlab
S = ndi.session.dir(my_session_path);
```
...
### Top Controls (Pipeline & Data Selection)
...
### Calculator List (The Center Panel)

This area displays the sequence of calculators in the current pipeline.
...
2. A file dialog will appear. Enter a name for your new Pipeline. Click **OK** to save.

The default saving location for your Pipelines will be:

```
~/Documents/MATLAB/Documents/NDI/My Pipelines
```

Within `My Pipelines`, there will be two directories:

- **`Calculator_Parameters`** — Contains subdirectories associated with each Calculator class once that Calculator has been added to any of your Pipelines. These subdirectories contain `.json` files corresponding to the Parameter Setup Code for each Calculator class. This automatically includes Default and Example Parameter Setup Code. Any user-created Parameter Setup Code will be saved here.
- **`Pipelines`** — Contains subdirectories associated with each created Pipeline. Each Pipeline subdirectory contains a `pipeline.json` file that stores the Calculator instances and most recently used Parameter Setup Code for loading and sharing.

The new pipeline will appear in the **Select Pipeline** dropdown. You can now add Calculator instances to it.
...
- **Default:** Calls the Calculator class' `default_search_for_input_parameters()` *(Read-only)*.
- **Example:** Based on the source code, often containing pre-filled queries derived from the Calculator class' `default_parameters_query` *(Editable)*.
...
The top window contains Documentation information defined by each Calculator class. You can view:

- **General** documentation on what operations the Calculator object performs.
- **Calculator Input Options**
- **Fields** contained by Output Documents.

### 6.2 The Code Editor

The large text box contains MATLAB code. You may view and/or edit the Example Parameter Setup Code, or create your own user-defined Parameter Setup Code. You are primarily defining two variables:

- **`thecalc`** — The calculator object.
- **`parameters`** — A structure defining inputs (usually via `ndi.query`).

Define your inputs based on the desired analysis you want the Calculator to perform. A Template Parameter Setup Code is provided as a starting point.

**Saving and managing Parameter Setup Code:**

- Click **Save As** to name and save your new Parameter Setup Code. It will be saved under the Calculator class subdirectory within `Calculator_Parameters`.
- Click **Refresh** to ensure both the Edit Calculator window and Pipeline Manager recognize new Parameter Setup Code. It should then appear in the dropdown menu.
- Click **Save** to save changes to existing Parameter Setup Code.
- Click **Delete** to remove the currently selected Parameter Setup Code.
- Click **Exit** to close the window and return to the Pipeline Manager.

### 6.3 Testing Your Code (The Commands Menu)

Before running the full pipeline, use the **Commands** dropdown in the Editor window to verify your code:

| Command | Description |
|---|---|
| **Try searching for inputs** | Runs your code and checks if `parameters.query` actually finds documents in the linked NDI session. |
...
| **Plot existing outputs** | Generates plots for results already in the database (useful for tweaking plot settings without re-calculating). You will be prompted to choose between individual figures or subplots. |
...
- **Plotting:** If "Figure Output" was checked, a popup will ask whether you want **Individual Figures** (one window per document) or **Subplots** (batched figures).
