...
If the comparison rules generated for the first mock document (`mock.1.comparison.json`) are applicable to all other test cases, you can copy it to the other mock comparison files using the following code:

```matlab
for i=2:myCalculator.numberOfSelfTests
    copyfile('mock.1.comparison.json',['mock.' int2str(i) '.comparison.json']);
end
```
